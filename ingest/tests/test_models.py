...
def test_latest_profile_can_be_lords_while_commons_service_is_retained():
    ...
    member = parse_member(parse_profile(fixture.profiles[1]), current=False)
    periods = service_periods(parse_history(fixture.histories[1]), TERM_START, AS_OF, current=False)
...
def test_long_continuous_source_period_is_clipped_without_losing_original_date():
    ...
    period = service_periods(parse_history({"id": 1, **history}), TERM_START, AS_OF, current=True)[
        0
    ]
...
def test_reentry_retains_a_real_service_gap():
    ...
    periods = service_periods(parse_history({"id": 1, **history}), TERM_START, AS_OF, current=True)
...
def test_disagreement_between_history_and_current_cohort_is_rejected():
    ...
    with pytest.raises(ImportValidationError, match="disagrees"):
        service_periods(parse_history({"id": 1, **history}), TERM_START, AS_OF, current=True)
...
def test_service_that_ceased_before_or_on_term_start_is_excluded(end):
    ...
    assert (
        service_periods(parse_history({"id": 1, **history}), TERM_START, AS_OF, current=False) == []
    )
...
def test_missing_or_invalid_service_start_fails(bad_date):
    ...
    with pytest.raises(ImportValidationError, match="invalid date"):
        service_periods(
            parse_history({"id": 1, "houseMembershipHistory": [entry]}),
            TERM_START,
            AS_OF,
            current=True,
        )
